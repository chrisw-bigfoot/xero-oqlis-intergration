...
def transform_balance_sheet(file_path: str, legal_entity: str = None, report_id: str = None, created_at: str = None, report_month: str = None, long_format: bool = False) -> pd.DataFrame:
    print(f"Transforming Balance Sheet from file: {file_path} ...")
    ...
    df.loc[df['Account'] == 'Net Assets', 'Section'] = 'Net Assets'
    df.loc[df['Section'] == 'Net Assets', 'Account'] = np.nan
    ...
    print(f"Finished transforming Balance Sheet. Resulting DataFrame has {len(df)} rows and {len(df.columns)} columns.")
    return df